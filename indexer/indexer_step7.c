...
int main(int argc, char *argv[]) {
    ...
    // verify crawler directory
    char testpath[512];
    snprintf(testpath, sizeof(testpath), "%s/.crawler", pagedir);
    FILE *test = fopen(testpath, "r");
    if (test == NULL) {
        fprintf(stderr, "[Error] '%s' is not a valid crawler directory (missing .crawler)\n", pagedir);
        return 1;
    }
    fclose(test);
    ...
    if (ht == NULL) {
        fprintf(stderr, "[Error] cannot open hashtable\n");
        return 1;
    }
    ...
    if (index_save(ht, indexnm) != 0) {
        fprintf(stderr, "[Error] index_save failed\n");
